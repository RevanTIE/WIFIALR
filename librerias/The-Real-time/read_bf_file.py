...
while True:
    ...
    p3 = p.plot(pen = 'b')
    """
    #fig = p.figure(figsize=(18, 15))
    p1 = p.subplot(311)
    p.xlabel("Packet index")
    p.ylabel("Subcarrier index")
    p1.set_title("Antenna A")
    p.colorbar()

    p2 = p.subplot(312)
    p.xlabel("Packet index")
    p.ylabel("Subcarrier index")
    p2.set_title("Antenna B")
    p.colorbar()

    p3 = p.subplot(313)
    p.xlabel("Packet index")
    p.ylabel("Subcarrier index")
    p3.set_title("Antenna C")
    p.colorbar()
    """
    ...
    while True:
        ...
        if code == 187:
            ...
            p1.setData(x, get_scaled_csi.db(abs(csi[0:29, 0, :].flatten())));
            p2.setData(x, get_scaled_csi.db(abs(csi[30:59, 1, :].flatten())));
            p3.setData(x, get_scaled_csi.db(abs(csi[60:89, 2, :].flatten())));
            """
